...
const Mail = () => {
    ...
    const handleSendEmailAndDelete = async (vehicleId, email) => {
        try {
            // Send email request
            await axios.post('http://localhost:4005/api/send-email', { email });
            ...
            toast.success('Email sent and vehicle deleted successfully');
        } catch (error) {
            toast.error('Failed to send email or delete vehicle');
                            ...
                            <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden dark:bg-neutral-800 dark:border-neutral-700">
                                <div className="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200 dark:border-neutral-700">
                                    <h3 className="text-center mb-4 text-5xl font-extrabold leading-none tracking-tight text-gray-900 md:text-5xl lg:text-5xl dark:text-white">
                                        <span className="text-blue-800 dark:text-blue-500">Vehicle</span> Alert
                                    </h3>
                                </div>
                                                ...
                                                <tr key={vehicle._id}>
                                                    ...
                                                    <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                        <button
                                                            ...
                                                            onClick={() => handleSendEmailAndDelete(vehicle._id, vehicle.owner ? vehicle.owner.email : '')}
                                                        >
                                                            Alert
                                                        </button>
                                                    </td>
